...
define(
	...
	function(Calculator, ScientificCalculator, withExponents, delay) {
	    ...
	    describe(
		    "Calculator Tests",
		    function() {
			// Write a Calculator class that matches this
			// specification
			...
			// Write a delay function that matches this
			// specification
			describe(
				"delay",
				function() {
					...
					var willAdd = delay(100, calculator,
						'add', [ 1, 1 ]);

					expect(willAdd).to.be
						.instanceOf(Promise);
				    ...
				    it(
					    "delays execution",
					    function() {
						expect(delay(1000, calculator,
							'add', [ 10, 5 ])).to.eventually
							.equal(15);
						expect(delay(500, calculator,
							'subtract', [ 9, 5 ])).to.eventually
							.equal(4);
					    });

				    it(
					    "cannot execute functions that do not exist",
					    function() {
						expect(delay(1000, calculator,
							'sqrt', [ 2, 2 ])).to.be.rejected;
					    });
				});
